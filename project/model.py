...
class BottleneckBlock(nn.Module):
    """Bottleneck block with improved residual connections for deeper networks."""
    expansion = 4
    ...
    def __init__(
        ...
        self.se = SqueezeExcitation(out_channels * self.expansion) if use_se else nn.Identity()
...
class SEResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.dropout = nn.Dropout2d(p=0.2)  # Increased from 0.1
        self.conv2 = nn.Conv2d(
            out_channels,
            out_channels,
            kernel_size=3,
...
class SEResNet(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(p=0.4)  # Increased from 0.3
...
class DeepSEResNet(nn.Module):
    """Deeper SE-ResNet with bottleneck blocks for CIFAR-100."""
    ...
    def __init__(
        ...
        # Using bottleneck blocks for deeper networks
        self.layer1, block_index = self._make_stage(
            base_width,
            base_width,
            blocks=blocks_per_stage[0],
        ...
        self.fc = nn.Linear(base_width * 32, num_classes)
        self._init_weights()
